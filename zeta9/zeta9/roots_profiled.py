...
import math, time
from sage.all import CyclotomicField, NumberField, QQ, RealField, ZZ, polygen, factor, matrix, vector
...
def _prime_cache_key1(P):
    return tuple(str(g) for g in P.gens())


def _prime_cache_key2(P):
    try:
        return tuple(str(g) for g in P.gens_reduced())
    except Exception:
        try:
            return tuple(str(g) for g in P.gens())
        except Exception:
...
def _ext_fac_signature(ext_fac):
    sig = []
    for item in ext_fac:
        Q, e = item
        try:
            qsig = tuple(str(g) for g in Q.gens_reduced())
        except Exception:
            try:
                qsig = tuple(str(g) for g in Q.gens())
            except Exception:
                qsig = str(Q)
        sig.append((qsig, int(e)))
    sig.sort()
    return tuple(sig)
...
def enumerate_candidate_ideals(coeffs_or_analysis, max_candidates=None, prec=80, field_data=None):
    ...
    for idx, choice_tuple in enumerate(product(*choice_lists)):
        I, summary = _build_ideal_from_choice(choice_tuple, K)
        out.append({"ideal": I, "local_choice_summary": summary})
...
def orbit_expand_roots(roots):
    try:
        ...
        return tuple(sorted(set(tuple(r) for r in roots)))
...
def simple_constructive_roots(coeffs, field_data=None, max_candidates=256, verbose=False):
    ...
    for idx, cand in enumerate(cands):
        I = cand["ideal"]
        pg = principal_generator_compat(I, proof=False, verbose=verbose)
...
__all__ = [
    ...
    "quick_screen_M_profiled", "_profile_summary",
]
...
def process_M(coeffs, prec=100, max_candidates=None, field_data=None):
    ...
    return {
        ...
        "candidates": [
            {"ideal": str(item["ideal"]), "local_choice_summary": [str(x) for x in item["local_choice_summary"]]}
