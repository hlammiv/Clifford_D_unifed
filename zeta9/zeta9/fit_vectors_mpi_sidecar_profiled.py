...
def _new_profile():
    return {
        ...
        "candidate_subset_cache_hits": 0,
        "candidate_subset_cache_misses": 0,
        "triples_processed": 0,
...
def _profile_add(dst: dict, src: dict):
...
def _target_cache_key(target: complex, eps: float, f: int):
    return (
        round(float(np.real(target)), 15),
        round(float(np.imag(target)), 15),
        float(eps),
        int(f),
    )
...
def _ensure_phase_sidecar(paths: dict, verbose=False, profile: dict | None = None, build_if_missing: bool = False):
    _t_sidecar0 = time.time()
    ...
    if os.path.exists(meta_path):
        ...
        if ok:
            if profile is not None:
                profile["sidecar_ensure"] += time.time() - _t_sidecar0
            return meta

    if not build_if_missing:
        raise FileNotFoundError(
            f"Phase sidecar not found for prefix {os.path.abspath(paths['phase_sidecar_dir']).removesuffix('.phase_sidecar')}. "
            f"Build it first with build_phase_sidecar.py or pass --build_sidecar_if_missing."
        )
...
def _get_sidecar_batch_cache(batch_idx: int, phase_meta: dict, batch_cache: OrderedDict, max_entries: int = 4, profile: dict | None = None):
    cached = batch_cache.get(batch_idx)
    if cached is not None:
        batch_cache.move_to_end(batch_idx)
        if profile is not None:
            profile["batch_cache_hits"] += 1
        return cached

    if profile is not None:
        profile["batch_cache_misses"] += 1
...
def _describe_candidates_from_sidecar(
    ...
    bd = _get_sidecar_batch_cache(batch_idx, phase_meta, batch_cache, profile=profile)
    off = bd["roots_off"]
    ...
    abs_ranges = tuple((a + l, a + r_) for (l, r_) in ranges if r_ > l)
    if not abs_ranges:
        return None
    return (int(batch_idx), abs_ranges)


def _descriptor_row_count(desc):
    if desc is None:
        return 0
    _, ranges = desc
    return int(sum(e - s for s, e in ranges))


def _coeff_row_from_descriptor(desc, batch_cache: OrderedDict, phase_meta: dict, which_range: int, offset_in_range: int):
    batch_idx, ranges = desc
    bd = _get_sidecar_batch_cache(batch_idx, phase_meta, batch_cache)
    s, e = ranges[which_range]
    return np.asarray(bd["roots_flat"][s + offset_in_range], dtype=np.int64)


def _complex_slices_from_descriptor(desc, batch_cache: OrderedDict, phase_meta: dict, inv_scale: float):
    if desc is None:
        return []
    batch_idx, ranges = desc
    bd = _get_sidecar_batch_cache(batch_idx, phase_meta, batch_cache)
    out = []
    for s, e in ranges:
        zre = np.asarray(bd["z_re_flat"][s:e], dtype=np.float64) * inv_scale
        zim = np.asarray(bd["z_im_flat"][s:e], dtype=np.float64) * inv_scale
        out.append((zre, zim))
    return out

def _get_candidate_subset_cached(
    y,
    coord: int,
    target: complex,
    eps: float,
    ...
    subset_cache_max_entries: int,
    profile: dict | None = None,
):
    key = (y, int(coord), _target_cache_key(target, eps, f))
    cached = subset_cache.get(key)
    if cached is not None:
        subset_cache.move_to_end(key)
        if profile is not None:
            profile["candidate_subset_cache_hits"] += 1
        return cached

    if profile is not None:
        profile["candidate_subset_cache_misses"] += 1
    desc = _describe_candidates_from_sidecar(
        y, locator, phase_meta, batch_cache, target, eps, f, profile=profile
    )
    subset_cache[key] = desc
    while len(subset_cache) > subset_cache_max_entries:
        subset_cache.popitem(last=False)
    return desc
...
def fit_vectors(
    *,
    ...
    candidate_subset_cache_size: int = 200000,
    verbose: bool = False,
    write_profile: bool = True,
    build_sidecar_if_missing: bool = False,
):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    size = comm.Get_size()
    rank_profile = _new_profile()
    _t_total0 = time.time()
    ...
    if rank == 0:
        ...
        phase_meta = _ensure_phase_sidecar(paths, verbose=verbose, profile=rank_profile, build_if_missing=build_sidecar_if_missing)
    else:
        nrows = None
        targets = None
        paths = None
    ...
    while start < nrows:
        ...
        rank_profile["triple_read"] += time.time() - _t0
        _t0 = time.time()
        needed = np.unique(triples.reshape((-1, 3)), axis=0)
        rank_profile["needed_unique"] += time.time() - _t0
        rank_profile["needed_y"] += int(needed.shape[0])
        _t0 = time.time()
        locator = _build_locator(needed, paths)
        rank_profile["locator_build"] += time.time() - _t0
        ...
        for t0 in range(0, local_targets.shape[0], int(targets_chunk_size)):
            ...
            for y in locator.keys():
                for j in range(t1 - t0):
                    for coord in range(3):
                        desc = _get_candidate_subset_cached(
                            y,
                            coord,
                            targ[j, coord],
                            eps,
                            ...
                            batch_cache,
                            subset_cache,
                            int(candidate_subset_cache_size),
                            profile=rank_profile,
                        )
                        if max_roots_per_Y is not None and desc is not None:
                            batch_idx, ranges = desc
                            remain = int(max_roots_per_Y)
                            new_ranges = []
                            for s, e in ranges:
                                if remain <= 0:
                                    break
                                take_here = min(remain, e - s)
                                if take_here > 0:
                                    new_ranges.append((s, s + take_here))
                                    remain -= take_here
                            desc = (batch_idx, tuple(new_ranges)) if new_ranges else None
                        candidate_cache[(j, y, coord)] = desc
                        rank_profile["candidate_rows_loaded"] += _descriptor_row_count(desc)
            rank_profile["candidate_load"] += time.time() - _t0
            rank_profile["candidate_cache_entries"] += len(candidate_cache)
            ...
            for tri in triples:
                ...
                for j in range(t1 - t0):
                    local_j = t0 + j
                    desc1 = candidate_cache[(j, Y1_t, 0)]
                    desc2 = candidate_cache[(j, Y2_t, 1)]
                    desc3 = candidate_cache[(j, Y3_t, 2)]
                    if desc1 is None or desc2 is None or desc3 is None:
                        continue
                    if _descriptor_row_count(desc1) == 0 or _descriptor_row_count(desc2) == 0 or _descriptor_row_count(desc3) == 0:
                        continue
                    d0, d1, d2 = targ[j, 0], targ[j, 1], targ[j, 2]
                    cur_best = local_best_dist[local_j]
                    cur_best_sq = cur_best * cur_best
                    z1_slices = _complex_slices_from_descriptor(desc1, batch_cache, phase_meta, inv_scale)
                    z2_slices = _complex_slices_from_descriptor(desc2, batch_cache, phase_meta, inv_scale)
                    z3_slices = _complex_slices_from_descriptor(desc3, batch_cache, phase_meta, inv_scale)
                    for r1_idx, (z1re, z1im) in enumerate(z1_slices):
                        for a in range(z1re.shape[0]):
                            za = complex(float(z1re[a]), float(z1im[a]))
                            da = abs(za - d0) ** 2
                            if da >= cur_best_sq:
                                continue
                            for r2_idx, (z2re, z2im) in enumerate(z2_slices):
                                for b in range(z2re.shape[0]):
                                    zb = complex(float(z2re[b]), float(z2im[b]))
                                    dab = da + abs(zb - d1) ** 2
                                    if dab >= cur_best_sq:
                                        continue
                                    for r3_idx, (z3re, z3im) in enumerate(z3_slices):
                                        for c in range(z3re.shape[0]):
                                            rank_profile["combinations_tested"] += 1
                                            zc = complex(float(z3re[c]), float(z3im[c]))
                                            dist = math.sqrt(dab + abs(zc - d2) ** 2)
                                            if dist < cur_best:
                                                cur_best = dist
                                                cur_best_sq = dist * dist
                                                local_best_dist[local_j] = dist
                                                local_best_u_coeffs[local_j, 0, :] = _coeff_row_from_descriptor(desc1, batch_cache, phase_meta, r1_idx, a)
                                                local_best_u_coeffs[local_j, 1, :] = _coeff_row_from_descriptor(desc2, batch_cache, phase_meta, r2_idx, b)
                                                local_best_u_coeffs[local_j, 2, :] = _coeff_row_from_descriptor(desc3, batch_cache, phase_meta, r3_idx, c)
            ...
            rank_profile["combination_loop"] += time.time() - _t0
            if verbose and rank == 0:
    ...
    rank_profile["gather"] += time.time() - _t0
    rank_profile["total"] += time.time() - _t_total0
    ...
    if rank == 0:
        ...
        profile_sum = _new_profile()
        for part in gathered:
            if "profile" in part:
                _profile_add(profile_sum, part["profile"])
        profile_by_rank = {str(i): gathered[i].get("profile", {}) for i in range(len(gathered))}
        profile_json = output_prefix + ".profile.json"
        profile_payload = {
            "sum": profile_sum,
            "by_rank": profile_by_rank,
        }
        if write_profile:
            _write_manifest(profile_json, profile_payload)
        summary = {
            ...
            "phase_sidecar_prebuilt": True,
            "profile_json": os.path.abspath(profile_json) if write_profile else None,
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--candidate_subset_cache_size", type=int, default=200000)
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--no_profile", action="store_true")
    parser.add_argument("--build_sidecar_if_missing", action="store_true")
    ...
    out = fit_vectors(
        ...
        candidate_subset_cache_size=args.candidate_subset_cache_size,
        verbose=not args.quiet,
        write_profile=not args.no_profile,
        build_sidecar_if_missing=args.build_sidecar_if_missing,
    )
