...
def fit_vectors(
    *,
    ...
    rank_profile = {
        ...
        "triples_processed": 0,
        "triples_with_nonempty_candidates": 0,
        "triples_skipped_empty": 0,
        "a_blocks": 0,
        "a_candidates_visited": 0,
        "a_pruned": 0,
        "b_blocks": 0,
        "b_candidates_visited": 0,
        "b_pruned": 0,
        "c_blocks": 0,
        "c_candidates_visited": 0,
        "c_pruned": 0,
                ...
                for j in range(t1_idx - t0_idx):
                    for coord in range(3):
                        desc = _candidate_desc_from_binned_sidecar(
                            y, locator, phase_meta, batch_cache, targ[j, coord], eps, f, profile=rank_profile
                        )
            ...
            for tri in triples:
                ...
                for j in range(t1_idx - t0_idx):
                    ...
                    if desc1 is None or desc2 is None or desc3 is None:
                        rank_profile["triples_skipped_empty"] += 1
                        continue

                    rank_profile["triples_with_nonempty_candidates"] += 1
                    ...
                    for base1, idx1_local, z1 in _iter_desc_z_and_indices(desc1, phase_meta, batch_cache, f, eps):
                        if z1.size == 0:
                            continue
                        rank_profile["a_blocks"] += 1
                        for a_idx, za in enumerate(z1):
                            rank_profile["a_candidates_visited"] += 1
                            da = abs(za - d0) ** 2
                            if da >= cur_best_sq:
                                rank_profile["a_pruned"] += int(z1.size - a_idx)
                                continue
                            ...
                            for base2, idx2_local, z2 in _iter_desc_z_and_indices(desc2, phase_meta, batch_cache, f, eps):
                                if z2.size == 0:
                                    continue
                                rank_profile["b_blocks"] += 1
                                for b_idx, zb in enumerate(z2):
                                    rank_profile["b_candidates_visited"] += 1
                                    dab = da + abs(zb - d1) ** 2
                                    if dab >= cur_best_sq:
                                        rank_profile["b_pruned"] += int(z2.size - b_idx)
                                        continue
                                    ...
                                    for base3, idx3_local, z3 in _iter_desc_z_and_indices(desc3, phase_meta, batch_cache, f, eps):
                                        if z3.size == 0:
                                            continue
                                        rank_profile["c_blocks"] += 1
                                        for c_idx, zc in enumerate(z3):
                                            rank_profile["c_candidates_visited"] += 1
                                            rank_profile["combinations_tested"] += 1
                                            dist = math.sqrt(dab + abs(zc - d2) ** 2)
                                            if dist >= cur_best:
                                                rank_profile["c_pruned"] += int(z3.size - c_idx)
                                                continue
                                            cur_best = dist
