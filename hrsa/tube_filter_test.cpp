// tube_filter_test.cpp - BFS over Cliff+D matrix set to K=4, then filter
// each level by various "near-R^Z" thresholds δ. Reports |G_K|, |G_K_tube(δ)|,
// and best Frob to test angles from each subset.
//
// "Near-R^Z" predicate at threshold δ:
//   max_{i≠j} |V[i][j]| < δ   AND   |V[2][2] - 1| < δ
// This is the tube around the R^Z manifold in U(3).
//
// Usage: ./tube_filter_test
// Produces a clean text table to stdout.
...
static double frob_dist_to_RZ(const CMat3& V, double theta) {
    ...
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) {
            cd ref = (i==j) ? (i==0?t00:(i==1?t11:t22)) : cd(0,0);
            cd d = V.m[i][j] - ref;
...
// "Near-R^Z" tube test: V is in tube iff max off-diagonal magnitude < δ
// and |V[2][2] - 1| < δ.
...
// Use std::string as the matrix key (raw bytes from 9 uint64s).
// std::string has built-in noexcept hash and equality, avoiding GCC 13 hashtable ctor warnings.
using MatKey = std::string;
static MatKey make_key(const CMat3& V) {
    constexpr double S = 1e7;  // 1e-7 rounding
    char buf[72];
    for (int i = 0; i < 3; ++i)
        for (int j = 0; j < 3; ++j) {
...
int main() {
    // Build Cliffords (same scaffolding as k3_exhaust_test.cpp).
    ...
    // BFS to K=4 with inline dedup.
    int MAX_K = 4;
    vector<vector<CMat3>> G(MAX_K + 1);
    unordered_map<MatKey, int> seen_global;  // key → level
    seen_global.reserve(5000000);
    G[0] = cliffords;
    ...
    auto report_level = [&](int K, double wall) {
        ...
        cout << "tube counts (max off-diag and |m22-1| both < δ):\n";
        ...
        cout << "\nbest Frob from full set vs tube(δ=0.5):\n";
        cout << "  theta       full           tube0.5        difference\n";
        for (double th : thetas) {
            double bf = 1e30, bt = 1e30;
            for (const auto& V : gK) {
                double f = frob_dist_to_RZ(V, th);
                if (f < bf) bf = f;
                if (tube_score(V) < 0.5 && f < bt) bt = f;
            }
            cout << "  " << setw(8) << th
                 << "  " << setw(13) << setprecision(10) << bf
                 << "  " << setw(13) << setprecision(10) << bt
                 << "  " << setw(11) << setprecision(3) << (bt - bf) << "\n";
        }
    };
    ...
    for (int K = 1; K <= MAX_K; ++K) {
        ...
        long long total_tasks = (long long)prev.size() * 2;
